      ...
      <div>
        <div className="debugControls">
          <ul>
            ...
            <li>
              <button onClick={this.handleTestEmitTEXT}>EDITOR TEXT</button>
            </li>
            <li>
              <button onClick={this.handleTestEmitAndDecode}>EDITOR STATE</button>
            </li>
          </ul>
        </div>
        <div className="debugZone" style={{ display: this.state.display }}>
          <button href="#" className="dante-debug-close close" onClick={this.toggleDisplay} />
          <div className="debugOutput">
            <h2>EDITOR OUTPUT</h2>
            {this.state.output.length > 0 ? <pre>{this.state.output}</pre> : undefined}
          </div>
        </div>
      </div>
    );
