...
class DanteImagePopover extends React.Component {
  ...
  render = () => {
    return (
      <AnchorStyle
        ref="image_popover"
        className={`dante-popover popover--Aligntooltip popover--top popover--animated ${
          this.state.show ? 'is-active' : undefined
        }`}
        ...
        <div className="popover-inner">
          <ul className="dante-menu-buttons">
            {this.state.buttons.map((item, i) => {
              return <DanteImagePopoverItem item={item} handleClick={this.handleClick} key={i} />;
            })}
          </ul>
        </div>
...
class DanteImagePopoverItem extends React.Component {
  ...
  render = () => {
    return (
      <li className={`dante-menu-button align-${this.props.item.type}`} onMouseDown={this.handleClick}>
        <span className={`tooltip-icon dante-icon`}>{Icons[`image${capitalize(this.props.item.type)}`]()}</span>
      </li>
    );
  };
}
