    ...
    return (
      <span>
        ...
        {!this.props.blockProps.getEditor().props.read_only ? (
          <a href="#" className={'graf--media-embed-close'} onClick={this.deleteSelf}>
            x
          </a>
        ) : null}
        ...
        >
          <strong className="markup--strong markup--mixtapeEmbed-strong">{this.state.embed_data.title}</strong>

          <em className="markup--em markup--mixtapeEmbed-em">{this.state.embed_data.description}</em>
        </a>

        <span contentEditable={false}>{this.state.embed_data.provider_url || this.state.embed_data.url}</span>
      </span>
    );
  }
}
