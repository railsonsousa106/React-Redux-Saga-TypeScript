    ...
    return (
      <div style={{ width: '100%', height: '100px', margin: '18px 0px 47px' }}>
        <div className="signature">
          <CardImage image={this.state.image} setImage={this.setImage} config={this.config} {...this.props} />
          <div
            className="text"
            style={{
...
class CardImage extends React.Component {
  ...
  render() {
    return (
      <div>
        <a href="#" contentEditable="false" onClick={this.clickOnFileUpload}>
          <img src={this.props.image} />
          <Loader toggle={this.state.loading} progress={this.state.loading_progress} />
        </a>
        <input type="file" style={{ display: 'none' }} ref="fileInput" onChange={this.handleFileInput} />
      </div>
    );
  }
}
